<div class="gshop-header-cart relative">
    <button type="button" class="header-icon theme-icon">
        <svg width="19" height="23" viewBox="0 0 22 25" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
        ...
        <span class="cart-counter badge bg-secondary rounded-circle p-0 {{ count($cartItems) > 0 ? '' : 'hidden' }}">
            x{{ count($cartItems) }}
        </span>
    </button>
    <div class="cart-box-wrapper">
        <div class="apt_cart_box theme-scrollbar">
            ...
            <div class="flex items-center justify-between mt-3">
                ...
                <span class="fw-semibold text-secondary sub-total-price">{{ number_format($subtotal, 2) }}€</span>
            </div>
            <div class="row items-center justify-between mt-3">
                <a href="{{ route('cart') }}" class="btn btn-primary btn-md mt-4 w-full"><span class="me-2"><i
                            class="fa-solid fa-credit-card"></i></span>CART</a>
            </div>
        </div>
    </div>
</div>
