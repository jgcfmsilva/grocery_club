...
@section('content')
    ...
    <form id="register-form" method="POST" action="{{ route('register') }}" enctype="multipart/form-data" class="max-w-4xl mx-auto bg-white p-6 rounded-xl shadow-md space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" required class="mt-1 block w-full rounded-md border-2 border-gray-300 bg-white focus:ring-indigo-500 focus:border-indigo-500 form-input-field py-2 px-4" />
            </div>

            <!-- Email -->
            <div>
                <label for="email" class="block text-sm font-medium form-input-label">Email</label>
                <input type="email" name="email" id="email" value="{{ old('email') }}" required class="mt-1 block w-full rounded-md border-2 border-gray-300 bg-white focus:ring-indigo-500 focus:border-indigo-500 form-input-field py-2 px-4" />
            </div>

            <!-- Password -->
            <div>
                <label for="password" class="block text-sm font-medium form-input-label">Password</label>
                <input type="password" name="password" id="password" value="{{ old('password') }}" required class="mt-1 block w-full rounded-md border-2 border-gray-300 bg-white focus:ring-indigo-500 focus:border-indigo-500 form-input-field py-2 px-4" />
            </div>

            <!-- Confirmar Password -->
            <div>
                <label for="password_confirmation" class="block text-sm font-medium form-input-label">Confirm Password</label>
                <input type="password" name="password_confirmation" id="password_confirmation" value="{{ old('password_confirmation') }}" required class="mt-1 block w-full rounded-md border-2 border-gray-300 bg-white focus:ring-indigo-500 focus:border-indigo-500 form-input-field py-2 px-4" />
            </div>

            <!-- Género -->
            <div>
                <label for="gender" class="block text-sm font-medium form-input-label">Gender</label>
                <select name="gender" id="gender" required class="mt-1 block w-full rounded-md border-2 border-gray-300 bg-white focus:ring-indigo-500 focus:border-indigo-500 form-input-field py-2 px-4">
            ...
            <div>
                ...
                <input type="text" name="nif" id="nif" value="{{ old('nif') }}" maxlength="9" class="mt-1 block w-full rounded-md border-2 border-gray-300 bg-white focus:ring-indigo-500 focus:border-indigo-500 form-input-field py-2 px-4" />
            </div>

            <!-- Endereço -->
            <div class="md:col-span-2">
                <label for="default_delivery_address" class="block text-sm font-medium form-input-label">Delivery Address</label>
                <input type="text" name="default_delivery_address" id="default_delivery_address" value="{{ old('default_delivery_address') }}" class="mt-1 block w-full rounded-md border-2 border-gray-300 bg-white focus:ring-indigo-500 focus:border-indigo-500 form-input-field py-2 px-4" />
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <div class="flex items-center space-x-4">
                    <div class="w-20 h-20 rounded-md overflow-hidden bg-gray-100 border-2 border-gray-300 flex items-center justify-center mr-4">
                        <img id="photoPreview" src="#" alt="Pré-visualização da foto" class="hidden w-full h-full object-cover rounded-md" />
