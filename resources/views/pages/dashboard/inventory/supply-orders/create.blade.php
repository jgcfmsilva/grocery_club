...
<div class="w-full px-0">
    <div class="flex flex-col md:flex-row items-center justify-between mb-8 w-full">
        ...
        <div class="flex gap-4 mt-4 md:mt-0 bg-gray-100 rounded-lg p-3">
            <a href="{{ route('dashboard.inventory.supply-orders.create', ['auto' => 1]) }}"
               class="bg-orange-500 hover:bg-orange-600 text-white font-semibold px-4 py-2 rounded-lg shadow transition duration-200 flex items-center gap-2 text-base">
                <i class="fas fa-magic"></i>
                Create Automatically
            </a>
            <a href="{{ route('dashboard.inventory.supply-orders.create') }}"
               class="bg-blue-500 hover:bg-blue-700 text-white font-semibold px-4 py-2 rounded-lg shadow transition duration-200 flex items-center gap-2 text-base">
                    ...
                    <div class="overflow-x-auto">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            @foreach($autoProducts as $i => $prod)
                                <div class="bg-gray-50 rounded-lg shadow p-4 flex flex-col items-center border border-gray-200">
    ...
    @else
        <form action="{{ route('dashboard.inventory.supply-orders.store') }}" method="POST" class="bg-white rounded-xl shadow-lg p-8 w-full">
            @csrf

            <div id="manual-products-list" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="manual-product-row items-end bg-gray-100 shadow-lg border-1 border-gray-100 rounded-lg p-4 flex flex-col gap-4">
                    <div class="flex flex-row gap-4 w-full">
                        <div class="w-2/3 min-w-0 mr-4 flex flex-col justify-end">
                        ...
                        <div class="w-1/3 min-w-0 flex flex-col justify-end">
                        ...
                        <div class="flex-shrink-0 flex items-end pl-4" style="height: 100%;">
