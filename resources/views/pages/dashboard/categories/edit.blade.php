...
<div class="container mx-auto">
    <div class="flex items-center mb-6">
        ...
        <h1 class="text-2xl font-bold ml-4">Edit Category</h1>
    </div>

    <form action="{{ route('dashboard.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="mt-6">
        @csrf
        @method('PUT')
        <div class="mb-4">
            <label for="name" class="block text-sm font-medium">Name</label>
            <input type="text" name="name" id="name" value="{{ $category->name }}" class="border border-gray-300 rounded px-4 py-2 w-full" required>
        </div>
        <div class="mb-8">
            ...
            <div class="flex items-center space-x-4">
                <div class="w-20 h-20 rounded-md overflow-hidden bg-gray-100 border-2 border-gray-300 flex items-center justify-center mr-4">
                    @if($category->image)
                        <img id="photoPreview" src="{{ $category->image_url }}" alt="Image Preview" class="w-full h-full object-cover rounded-md" />
                    @else
